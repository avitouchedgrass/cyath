...
class RetroAudioEngine {
  ...
  // Heavy acoustic stamp impact with deep parchment thud
  public playWaxStampThump() {
    if (this.isMuted) return;
    try {
      this.initCtx();
      if (!this.ctx) return;
      ...
      // 1. Heavy wooden desk sub-bass thud
      ...
      // 2. Brass-on-wax high-pressure surface smack
  ...
  // Smooth desk drawer slide sound
  ...
  public startSanctuarySoundscape() {
    ...
    try {
      ...
      // Noticeably boosted from 0.04 to 0.12 for immersive desk audio
