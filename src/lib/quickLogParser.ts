...
export function parseQuickLog(
  ...
  const navTargets: Record<string, { path: string; label: string }> = {
    ...
    recipes: { path: '/recipes', label: 'Whole-Food Recipes Catalog' },
    meals: { path: '/recipes', label: 'Whole-Food Recipes Catalog' },
    food: { path: '/recipes', label: 'Whole-Food Recipes Catalog' },
