...
import { TacticalStatusDock } from '@/components/dashboard/TacticalStatusDock';
import { CommandProtocolCard } from '@/components/dashboard/CommandProtocolCard';
import { StoveSageBriefingCard } from '@/components/dashboard/StoveSageBriefingCard';
import { CircadianHorizonBar } from '@/components/dashboard/CircadianHorizonBar';
import { SmartRitualCard } from '@/components/dashboard/SmartRitualCard';
  ...
  const routineWindow = useMemo(() => {
    ...
    if (hour < 11) {
      return {
        ...
        description: 'Drink 500ml water, get outdoor sunlight, and anchor amino acids.',
      };
    } else if (hour < 17) {
      return {
        badge: 'AFTERNOON',
        title: 'Deep Focus Block',
        description: 'Single-task priority goals, maintain hydration, and take a post-meal stroll.',
      };
    } else {
      return {
        badge: 'EVENING',
        title: 'Restful Wind-Down',
        description: 'Dim bright monitors, honor caffeine cutoff, and prepare slow-wave recovery.',
      };
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#F4F0EA] text-[#1A3629] transition-colors duration-300 flex flex-col selection:bg-[#1A3629] selection:text-[#FFFDF9]">
      ...
      <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-10 pt-24 pb-20 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b-2 border-[#1A3629]/15 pb-4">
          <div>
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border-2 border-[#1A3629] bg-[#FFFDF9] text-[#1A3629] font-mono text-xs font-bold hover:bg-[#1A3629] hover:text-[#FFFDF9] shadow-[2px_2px_0px_#1A3629] transition-all cursor-pointer"
            >
              <span>◷</span>
        ...
        {/* Asymmetric Two-Column Command Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* ========================================================= */}
          {/* LEFT COLUMN: Hero Protocol, Habit Engine & Meal Logistics */}
          {/* ========================================================= */}
          <div className="lg:col-span-8 flex flex-col gap-6">
            
            {/* Circadian Energy Engine Horizon */}
            <CircadianHorizonBar />

            {/* Time-Aware Desk Ritual Dock */}
            <SmartRitualCard />

            {/* Daily Protocol Hero */}
            <CommandProtocolCard />

            {/* 2. Consistency Heatmap (Toggle-able 7 vs 28 Days) */}
            <div className="border-2 border-[#1A3629] bg-[#FFFDF9] rounded-2xl p-5 sm:p-6 shadow-[3px_3px_0px_#1A3629]">
              ...
              {!isViewingToday && (
                <div className="mt-3 pt-3 border-t border-[#1A3629]/15 flex items-center justify-between text-xs font-mono text-[#92400E] bg-[#FEF3C7]/60 px-3 py-2 rounded-lg border border-[#D97706]/30">
                  <span>Viewing past log for {currentDate}. Progress edits are restricted to today.</span>
                  <button
                    type="button"
                    onClick={() => {
                      retroAudio.playBlip();
            ...
            <div className="border-2 border-[#1A3629] bg-[#FFFDF9] rounded-2xl p-5 sm:p-6 shadow-[3px_3px_0px_#1A3629] flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5 bg-[#FAF6EE] p-3 rounded-xl border border-[#1A3629]/15">
                <div className="flex items-center justify-between text-xs font-mono">
                  <span className="font-bold text-[#1A3629]">Habit Calibration</span>
              ...
              {/* 100% Completion Banner - Strictly Zero Emojis */}
              {completedCount === habits.length && habits.length > 0 && (
                <div className="p-3 rounded-xl border-2 border-[#10B981] bg-[#ECFDF5] text-[#065F46] flex items-center justify-between shadow-xs">
                  <span className="text-xs font-cabinet font-bold">
                    All habits checked · Daily routine calibrated
                  </span>
          ...
          {/* ========================================================= */}
          {/* RIGHT COLUMN: Telemetry Cockpit, AI Advisor & Biometrics   */}
          {/* ========================================================= */}
          <div className="lg:col-span-4 flex flex-col gap-6">
            
            {/* 1. Day Telemetry & Desk Ritual Command Dock */}
            <TacticalStatusDock />

            {/* 2. StoveSage Intelligence Advisory */}
            <StoveSageBriefingCard />

            {/* 3. Routine Focus & Daily Observation Notes */}
            <div className="border-2 border-[#1A3629] bg-[#FFFDF9] rounded-2xl p-5 shadow-[3px_3px_0px_#1A3629] flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-[#1A3629]/15 pb-2">
                <h2 className="font-fraunces font-bold text-base text-[#1A3629]">
                  {routineWindow.title}
                </h2>
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder={isViewingToday ? "Quick energy reflections or protocol notes..." : "Read-only notes for past day."}
                  rows={2}
            ...
            {/* 4. Consolidated Biometrics & Energy */}
            <div className="border-2 border-[#1A3629] bg-[#FFFDF9] rounded-2xl p-5 shadow-[3px_3px_0px_#1A3629] flex flex-col gap-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[15, 30, 45].map((amt) => (
                    <button
                      ...
                      className={`border rounded-lg h-9 text-xs font-mono font-bold transition-all active:scale-95 flex items-center justify-center ${
                        !isViewingToday
              ...
              <div className="p-3 rounded-xl bg-[#FAF6EE]/70 border border-[#1A3629]/15 flex flex-col gap-2">
                <div className="flex items-center justify-between text-xs font-mono">
                  <span className="font-bold text-[#1A3629]">Hydration</span>
              ...
              <div className="p-3 rounded-xl bg-[#FAF6EE]/70 border border-[#1A3629]/15 flex flex-col gap-2">
                <div className="flex items-center justify-between text-xs font-mono">
                  <span className="font-bold text-[#1A3629]">Energy Level</span>
              ...
              <div className="p-3 rounded-xl bg-[#FAF6EE]/70 border border-[#1A3629]/15 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`flex-1 h-9 rounded-lg border text-xs font-mono font-bold transition-all active:scale-95 flex items-center justify-center ${
                      !isViewingToday
                        ? 'border-[#1A3629]/10 bg-gray-100 text-[#1A3629]/40 cursor-not-allowed'
                        : 'border-[#1A3629]/30 hover:border-[#1A3629] hover:bg-[#1A3629] hover:text-[#FFFDF9] bg-white text-[#1A3629] cursor-pointer shadow-xs'
                    }`}
